...
class API(object):
    ...
    def play(self, id):
        ...
        url = self._session.head(data['dashManifestUrl'].replace('manifest=mpeg-dash', 'manifest=m3u'), allow_redirects=True).url
        if 'not-in-oz' in url.lower() or '10play_unsupported' in url.lower():
            # some ips dont work with 10-selector.global.ssl.fastly.net
            # user lower quality dai for them (like the website)
            # TODO: this only gets 960x540, app can get 720p with more payload data to /streams endpoint
